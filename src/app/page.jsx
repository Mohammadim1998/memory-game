          ...
          <div className="space-y-8 text-center">
            ...
            <div className="p-6 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 rounded-3xl shadow-2xl">
              ...
              <p className="text-white text-xl mt-4 font-medium">
                وضعیت فعلی: {isExpanded ? "تمام صفحه" : "کوچک شده (روش رسمی ایتا)"}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6 max-w-lg mx-auto">
              <button onClick={handleClose} className="px-8 py-5 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 transition shadow-lg">
                بستن برنامک
              </button>
              <button onClick={handleShowConfirm} className="px-8 py-5 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition shadow-lg">
                تأیید (Confirm)
              </button>
              <button onClick={handleShowAlert} className="px-8 py-5 bg-purple-600 text-white font-bold rounded-xl hover:bg-purple-700 transition shadow-lg">
                هشدار (Alert)
              </button>
              <button onClick={handleOpenLink} className="px-8 py-5 bg-green-600 text-white font-bold rounded-xl hover:bg-green-700 transition shadow-lg">
                باز کردن لینک
              </button>
            </div>

            <div className="mt-8 p-6 bg-gray-900 text-white rounded-2xl text-right">
              <p className="font-medium">آیدی کاربر: {window.Eitaa?.WebApp?.initDataUnsafe?.user?.id}</p>
              <p className="font-medium">نام: {window.Eitaa?.WebApp?.initDataUnsafe?.user?.first_name}</p>
            </div>
          </div>
        ) : (
          <div className="text-center text-gray-600 space-y-4">
            <p className="text-2xl">در حال بارگذاری SDK ایتا...</p>
            <p className="text-lg">این صفحه فقط داخل پیام‌رسان <strong>ایتا</strong> کار می‌کند.</p>
          </div>
        )}
      </main>
    </>
